...
{		
    ...
    Cmt2300_WriteReg(CMT2300_CUS_MODE_CTL, CMT2300_GO_RX);
		portEXIT_CRITICAL();
    return 1;//Cmt2300_WaitChipStatus(CMT2300_STA_RX);
}
...
* FIFO_RX_TX_SEL在作为一个 64-byte FIFO 时可使用0：用作 RX FIFO， 1：用作 TX FIFO。
* SPI_FIFO_RD_WR_SEL 配置成 0（SPI 读 FIFO 模式）
* *********************************************************/
...
* FIFO_RX_TX_SEL在作为一个 64-byte FIFO 时可使用0：用作 RX FIFO， 1：用作 TX FIFO。
* SPI_FIFO_RD_WR_SEL 配置成 1（SPI 写 FIFO 模式）
* *********************************************************/
...
//		Cmt2300_EnableAntennaSwitch(0);
    Cmt2300_ConfigGpio(CMT2300_GPIO3_SEL_INT2 |
	                     CMT2300_GPIO2_SEL_DOUT |
	                     CMT2300_GPIO1_SEL_DIN);
//	  Cmt2300_GoSleep();
	  Cmt2300_ClearFifo();
