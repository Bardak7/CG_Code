...
/**
  * @brief  STMFLASH_ReadHalfWord  读取指定地址的半字(16位数据)
  * @param  faddr:读地址 
  * @retval 对应数据.
  */ 
...
/**
  * @brief  STMFLASH_Erase_Word  伪擦除一个字数据 把这两个半字写0 可直接写 用于清除保存的地址等
...
/**
  * @brief  STMFLASH_Write_NoCheck  不检查的写入
  * @param 	WriteAddr:起始地址
	*					pBuffer:数据指针
	*					NumToWrite:半字(16位)数
  * @retval None
...
/**
  * @brief  STMFLASH_Write 从指定地址开始写入指定长度的数据
...
void STMFLASH_Write(u32 WriteAddr,u16 *pBuffer,u16 NumToWrite)	
{
	...
	{	
		...
		{
			if(STMFLASH_BUF[secoff+i]!=0XFFFF)break;//检查是否需要擦除
		}
		if(i<secremain)				//需要擦除
		{
			portENTER_CRITICAL();
...
/**
  * @brief  STMFLASH_Read 从指定地址开始读出指定长度的数据
  * @param	ReadAddr:起始地址
	*					pBuffer:数据指针
	*					NumToWrite:半字(16位)数
  * @retval None
